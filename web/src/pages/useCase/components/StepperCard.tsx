...
export const StepperCard: React.FC<Props> = ({ steps, currentStep }) => {
    ...
    return (
      <div className="flex flex-row" key={item.screenId}>
        <div className="flex flex-col">
          <div
            className="rounded-full h-7 w-7 p-3.5 ring-2 border-2 border-white dark:border-theme-darkgrey ring-theme-lightgrey dark:ring-theme-black mx-2 transition transition-all duration-300 "
            style={{ background: currentStepIsEqual ? 'rgb(146, 227, 169)' : 'rgb(201, 237, 211)' }}
          />
          {idx !== progress.length - 1 && (
            <div className="border-l-2 border-theme-lightgrey dark:border-theme-black border-rounded h-full m-auto" />
          )}
        </div>
  ...
  return (
    <motion.div className="flex flex-col bg-white dark:bg-theme-darkgrey rounded-lg p-4 h-auto shadow mb-4">
